...
def oracle(df_t, num_inf, epsilon):
    ...
    Input: df_t -- the graph to process
...
RESULTS_FOLDER_NAME = f"eps{epsilon}-2pparams{PRESERVE_PARAMS}"
if not os.path.exists(RESULTS_FOLDER_NAME):
    os.makedirs(RESULTS_FOLDER_NAME)
RESULTS_PICKLE_NAME = f"results_eps{epsilon}.pkl"
...
for sigma in sigma_arr:
    for c in c_arr:
        RESULTS_PICKLE_NAME = f"results_eps{epsilon}_sigma{sigma}_c{c}.pkl"
        for t in tqdm(np.sort(np.unique(df_friend["day"]))[50:55],
                      desc="Iterating through the dyn net",
                      file=sys.stdout):
            t_start = time.time()
            ...
            # m_inv[]
            # Finding values corresponding to this day
            ...
            with open(os.path.join(RESULTS_FOLDER_NAME, f"{t}.pkl"),
                      "wb") as f:
                pickle.dump({"m_inv": m_inv, "df": df_t, "b": b}, f)

        # Saving results
        with open(os.path.join(RESULTS_FOLDER_NAME, RESULTS_PICKLE_NAME),
                  "wb") as f:
